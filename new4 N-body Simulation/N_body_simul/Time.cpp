...
{
	tick = round((tick_)*pow(10, 15))*pow(10, -15);	//double error fix for range 10^-15
}
...
void Time::ru(double duration, Set* set_all_)
{
	...
	double fir_t = cur_t;
	double ne_t = cur_t; //next time
	
	while (true)
	{
		double temp_t = round((cur_t + tick)*pow(10, 15))*pow(10, -15);// double error fix
		ne_t = (cur_t + tick < duration + fir_t ? temp_t : duration + fir_t);	//time tick & checking operation for last time///////////////////////////////////¼öÁ¤
